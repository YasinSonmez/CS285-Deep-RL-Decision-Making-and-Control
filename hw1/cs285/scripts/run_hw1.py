...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          idx = range(len(replay_buffer.obs)) # List all possible indexes
          idx_batch = np.random.choice(idx, params['train_batch_size']) # Sample random indexes
          ob_batch, ac_batch = ptu.from_numpy(replay_buffer.obs[idx_batch]), ptu.from_numpy(replay_buffer.acs[idx_batch])
